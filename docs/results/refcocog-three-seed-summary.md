...
The modality controls behaved as expected. Averaged over all test examples and seeds, correct minus image-shuffle was `+48.7 pp` for A4 and `+48.6 pp` for S4. Correct minus text-shuffle was `+22.6 pp` and `+22.4 pp`, respectively. The raw per-example tensors and JSON analysis remain on the persistent pod under `runs/publishable-eval/`; this file is the version-controlled result record.
